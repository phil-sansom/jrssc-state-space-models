...
m0 = c(6, 0, 3.6, 1.0, 1.3, 0.7, rep(0,p), c( 1.8,-1.3, 0.7,-0.3, 0.1), 0, rep(0,p))
C0 = diag(c(1, 2e-3, 1, 1.5, 0.9, 1.3, rep(10,p), rep(0.2,p), 10, rep(0.2,p))^2)
...
log.prior = function (psi) {
    ...
    wdelta = dnorm(psi$wdelta, 0,  12, log = TRUE)
    pdelta = dbeta(    pdelta, 1,   1, log = TRUE)
    cdate  = dunif(psi$cdate , 0, 365, log = TRUE)
    clen   = dunif(psi$clen  , 0, 365, log = TRUE)
    cprop  = dbeta(    cprop , 1,   1, log = TRUE)

    return(sum(v,wmu,wbeta,wphi,wdelta,pdelta,cdate,clen,cprop))
...
fit.model = function (psi, y, n, k, p, m0, C0, omega) {
    ...
    W  = diag(c(wmu,wbeta,rep(wmu,2*kk),1,rep(0,p-1),rep(wphi,p),0,rep(wdelta,p)))
    ...
    model = make.model(y, n, k, p, V, W, Wx, omega, dp = pdelta, Bp = Bp)
