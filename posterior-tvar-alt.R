########################################
## Posterior Traces: TVAR Alternative ##
########################################
...
xlims = list(c(-19,+1), c(-21,-3), c(-37,-19),c(-1.5,+2.5),c(0,4),c(-2,+2),
             c(-27,-9),c(-28,-4),c(0.999,1.0),c(0,365),c(0,365), c(0,1))
xbreaks = list(seq(floor(min(samples[,1,])),ceiling(max(samples[,1,])),+1),
               ...
               seq(floor(min(samples[,8,])),ceiling(max(samples[,8,])),+1.0),
               seq(0.998,+1.0,+0.00001), seq(0,370,10), seq(0,370,10),
               seq(0.0,1.0,0.05))

## Open device
postscript(file = "posterior-tvar-alt.eps", width = height, height = width,
