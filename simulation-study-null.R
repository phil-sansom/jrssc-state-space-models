...
## Observations
y = mu + psi[1,] + psi[3,] + x + rnorm(n, 0, v)
...
m0 = c(0,0,rep(0,4),rep(0,p),0.0,rep(0,p-1))
C0 = diag(c(5,0.002,rep(5,4),rep(10,p),rep(1,p))^2)
dim(m0) = c(length(m0),1)

## Observation and evolution variances
V = matrix(v^2, 1, 1)
W = diag(c(wmu,wbeta,rep(wpsi,4),1,rep(0,p-1),rep(wphi,p))^2)
Wx = function(t) wx^2

## Fit model
model  = make.model(y, 2, c(1,2), p, V, W, Wx)
...
## Open eps device
postscript(file = "sim-null-mu.eps", width = width, height = height,
...
## Open eps device
postscript(file = "sim-null-beta.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-null-psi1.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-null-psi2.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Coefficients
## Open eps device
postscript(file = "sim-null-phi.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-null-amplitude.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-null-phase.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
