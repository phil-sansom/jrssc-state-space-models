...
wdelta = 5e-1
pdelta = 0.995
delta  = rnorm(n, 0, wdelta)
for (i in 2:n)
    delta[i] = pdelta*delta[i-1] + delta[i]
...
m0 = c(0,0,rep(0,4),rep(0,p),0.5,rep(0,p-1),0)
C0 = diag(c(5,wbeta,rep(5,4),rep(1,p),rep(1,p),10)^2)
dim(m0) = c(length(m0),1)

## Observation and evolution variances
V = matrix(v^2, 1, 1)
W = diag(c(wmu,wbeta,rep(wpsi,4),1,rep(0,p-1),rep(wphi,p),wdelta)^2)
Wx = function(t) wx^2

## Fit model
model = make.model(y, 2, c(1,2), p, V, W, Wx, omega, pdelta, Bn)
...
## Open eps device
postscript(file = "sim-mean-delta.eps", width = width, height = height,
...
plot(delta, type = "l", col = "red",  xaxt = "n",
     xlim = c(0,n), ylim = c(-12.5,+12.5), xaxs = "i", yaxs = "i")
lines(fit, lwd = 2)
...
## Open eps device
postscript(file = "sim-mean-mu.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-beta.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-psi1.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-psi2.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Coefficients
## Open eps device
postscript(file = "sim-mean-phi.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-delta-mean.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-amplitude.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
...
## Open eps device
postscript(file = "sim-mean-phase.eps", width = width, height = height,
           pointsize = 8, paper = "special", horizontal = FALSE)
